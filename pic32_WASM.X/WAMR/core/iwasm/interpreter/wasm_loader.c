                        ...
                        uint32 error_buf_size)
{
    ...
    /* check function type */
    if (!wasm_type_equal(expected_tag_type, (WASMType*)tag->tag_type, module->types,
                         module->type_count)) {
    ...
    if (!wasm_runtime_is_built_in_module(sub_module_name)) {
        ...
        if (sub_module) {
            ...
            WASMTag *linked_tag = wasm_loader_resolve_tag(
                sub_module_name, tag_name, (WASMType*)declare_tag_type,
...
orcjit_thread_callback(void *arg)
{
    ...
    for (i = group_idx; i < func_count;
        ...
        if (error != LLVMErrorSuccess) {
            char *err_msg = LLVMGetErrorMessage(error);
            LOG_ERROR("failed to compile llvm jit function %u: %s\n\r", i, err_msg);
            LLVMDisposeErrorMessage(err_msg);
            break;
        }
