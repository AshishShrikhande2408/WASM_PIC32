...
#define UART2_RX_QUEUE_SIZE 64U
#define UART2_TX_QUEUE_SIZE 64U
...
void Uart2Initialise(uint32_t baud)
{
    ...
    s_UartTxQueue = xQueueCreate(UART2_TX_QUEUE_SIZE, sizeof(uint8_t));
        ...
        // Set TRIS directions
    TRISCbits.TRISC2 = 0;   // RPC2 (U2TX) as output
    TRISCbits.TRISC3 = 1;   // RPC3 (U2RX) as input
    
    ANSELCbits.ANSC2 = 0;   // Disable analog on RPC2 (AN21)
    ANSELCbits.ANSC3 = 0;   // Disable analog on RPC3 (AN20)
    ...
    // Set PPS
    RPC2R = 0b0010;     // RPC2 = U2TX
    U2RXR = 0b1100;     // U2RX = RPC3
